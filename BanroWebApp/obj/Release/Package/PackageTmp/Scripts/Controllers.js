app.controller('CtrlHome', function (FactoryHome, $scope, $timeout) {
    ...
    $scope.udpateEmployed = function () {
        ...
        console.log("Object :", JSON.stringify($scope.Employed));
    }
    ...
    $scope.selectedEmployed = function (employed) {
        ...
        $scope.dateEmployed = employed.datenaiss
    ...
    $scope.AddChild = function (categ) {
        ...
        if ($scope.nameChild != "" && $scope.dateChild != "" && $scope.sexChild != "" && $scope.pictureChild != "") {
    ...
    $scope.editPartner = function (conjoint) {
        $scope.titleConjoint = "Modify Spouse";
        ...
        if ($scope.IsSearch==true) {
            ...
            if ($scope.roleUser=="user") {
                ...
                if ($scope.nameSucc == $scope.idAuthSucc) {
    ...
    $scope.addEmployed = function () {
        ...
        if ($scope.nameBene != undefined && $scope.sexBene != undefined && $scope.phoneBene != undefined && $scope.cbo_succ != undefined && $scope.cbo_depart != undefined && $scope.dateEmployed != undefined) {
    ...
    $scope.EditConjoint = function () {
        if ($scope.namePartner.trim() != "" && $scope.sexPartner.trim() != "" && $scope.phonePartner.trim() != "" && $scope.datenaisPartner.trim() != "") {
            $scope.conjoint = $scope.tabPartner[0];
            $scope.conjoint.name = $scope.namePartner;
            $scope.conjoint.sex = $scope.sexPartner;
            $scope.conjoint.phone = $scope.phonePartner;
            $scope.conjoint.datenais = $scope.datenaisPartner;
    ...
    $scope.cleanConjoint = function () {
        $scope.titleConjoint="New Spouse"
    ...
    $scope.addVisitor = function () {
        ...
        if ($scope.nameVisitor!=undefined && $scope.sexVisitor!=undefined && $scope.phoneVisitor!=undefined && document.querySelector('#pict_visitor').src!=$scope.imageBasic) {
            $scope.userVisitor = {
            ...
            alert("Replissez le(s) champ(s) vide(s)...");
        }
    }
    $scope.isVisible = false;
    ...
    $scope.selectedEmployedBCmd = function (employed) {
        switch ($scope.cboCategBeneficiaire.toString().trim()) {
            case 'Employee':
            ...
            case 'Spouse':
                ...
                if (employed.partner.ID_Succursale==null) {
                    ...
                    $scope.picturePrinter = employed.partner.picture;
                    ...
                    $scope.nameEmployedBC = employed.partner.name;
                    $scope.cboEtatCivilBC = employed.partner.CivilStatus;
                    $scope.matrAgent = employed.partner.id;
                ...
                break;
            case 'Child':
                $scope.picturePrinter = employed.Childs[0].picture;
                $scope.employedBC = employed.Childs[0];
                $scope.entreprise = employed.ID_Succursale;
                $scope.nameEmployedBC = employed.Childs[0].name;
                ...
                var old = employed.Childs[0].datenais.toString().trim().substring((employed.Childs[0].datenais.toString().trim().lastIndexOf('/') + 1));
                old = new Date().getFullYear() - old;
                console.log("OLD EMPLOYED :" + old);
                $scope.oldEmployed = old;
                document.querySelector('#pictureEmployedBC').src = "../../images/" + employed.Childs[0].picture;
            ...
            case "Visitor":
                ...
                $scope.entreprise = employed.CompanyName;
                $scope.nameEmployedBC = employed.name;
    ...
    $scope.addBonCommand = function () {
        ...
        if ($scope.idHospital!="" && $scope.cboApprouve!=undefined && $scope.datedeb!=undefined && $scope.datefin!=undefined) {
            ...
            $scope.bonCommand = {
                ...
                C_motif:$scope.tmotif.toString().trim()
            };
    ...
    $scope.SearchBonCommand = function (categ) {
        if (categ == "Master") {
            ...
            FactoryHome.getListBonCommand(name, category).then(function (response) {
                ...
                $scope.tabSearchEmployed = response;
                console.log("Child :",JSON.stringify(response));
                //if (category=="Child") {
...
//            $scope.cboFacture = "Employee";
            ...
            browser.onchange = function () {
                ...
                if (extension[1]=="csv") {
                    if (browser.files[0] != null) {
                        ...
                        FactoryHome.setSendFileCSV(browser.files[0]).then(function (response) {
                            if (response.toString().trim() == "200") {
        ...
        if (date_begin != "" && date_end != "") {
            if (date_begin < date_end) {
                ...
                angular.forEach($scope.tabBCmdFacture, function (value, key) {
                    ...
                    console.log("Date Trie :", currentDate.toLocaleDateString());
                    console.log("Date Begin :", date_begin.toLocaleDateString());
                    console.log("Date End :", date_end.toLocaleDateString());
    ...
    $scope.checkedDepartment = function (department) {
        ...
        console.log(JSON.stringify($scope.LibDeparts));
    }
    $scope.sendExportRMVD = function () {
        
        var date_begin = document.querySelector('#dateFrom').value.toString().trim();
        var date_end = document.querySelector('#dateTo').value.toString().trim();
